...
namespace Core {
	...
	std::string MemoryClass::ReadString(uintptr_t Addr) {
		...
		while (bytesRead < bufferSize) {
			char character = 0;
			size_t read = 0;
			NTSTATUS status = ZwReadVirtualMemory(ProcHandle, (LPVOID)(Addr + bytesRead), &character, sizeof(char), &read);
		...
		if (!success || bytesRead == 0) { return ""; }
		if (bytesRead < bufferSize) buffer[bytesRead] = '\0';
