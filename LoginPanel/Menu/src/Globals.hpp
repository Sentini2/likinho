...
class cColors {
public:
	ImVec4 Base = ImColor( 180, 180, 180 ); //75, 70, 175 - 130, 151, 248
...
// ============ LANGUAGE SYSTEM ============
inline int g_Language = 0; // 0 = English, 1 = Portuguese
inline const char* Lang(const char* en, const char* pt) {
...
// Star structure for galaxy animation
struct Star {
	ImVec2 pos;
	float size;
	float brightness;
...
// Global galaxy variables
inline std::vector<Star> galaxyStars;
inline Theme currentTheme = Theme::LiKinho;
inline ThemeColors activeTheme;
inline float animTime = 0.0f;
inline std::string executorName = "LIKINHO EXECUTOR";
inline std::string themeDisplayName = "LiKinho Theme";
...
public:
	...
	ID3D11ShaderResourceView * NpcImage = nullptr;
...
public:
	...
	int iTabCount = 0;
	float TabAlpha = 0.f;
	int iCurrentPage = 8; // Login page (shifted after adding Premium)
	float TabAdd = 0.f;
	bool IsLogged = true;
