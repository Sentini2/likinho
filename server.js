...
async function getSettings() {
    try {
        ...
        if (!settings) {
            console.log('[SETTINGS] "main" settings not found, creating default...');
            settings = await Setting.create({ key: 'main' });
        }
        return settings;
    } catch (e) {
        console.error('[SETTINGS] Error fetching settings:', e);
        // Return a fresh object if DB fails (shouldn't happen with mongoose connected)
        return new Setting({ key: 'main' });
    }
}
...
app.post('/api/admin/update/upload', adminAuth, upload.single('exe'), async (req, res) => {
    try {
        ...
        if (saved) {
            console.log(`[UPDATE] Persistent settings updated: v${newVersion} | ${fileUrl}`);
        }
